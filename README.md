...
collaboration contract is implemented; 3D, a general SDK and custom app builds remain reserved.
...
Open <http://127.0.0.1:5173>. Run `pnpm check` to verify formatting, lint, types, tests and
production builds. Use `pnpm format` to format files.

## Structure

```text
apps/
  video/            React + Vite video editor
...
packages/
  ...
  sdk/              Reserved app integration helpers
...
Use `pnpm build && pnpm start` to serve the preset build. Chrome is required for thumbnails and
