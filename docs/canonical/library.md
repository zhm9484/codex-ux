...
icons for folders, images, video, audio, fonts, documents and code. Arrow keys select results, Enter
attaches, and Escape dismisses without closing the composer. Chinese input composition does not
trigger selection. Inline references are atomic editable tokens with filename/path metadata;
input-local undo/redo restores text and references together. Clicking a token previews the
attachment.
